 ...
 *
 */
public class DSOForDataSetActivity extends Activity implements OnClickListener,
		Renderer {
	ImageView imgSource, imgDealed;
	Button start, stop;
	String vocPath, calibrationPath, ImgPath, ImgPath2;
	LinearLayout linear;
	ArrayList<Integer> imageLeftArray;
	ArrayList<Integer> imageRightArray;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		linear.addView(mGLSurfaceView, new LinearLayout.LayoutParams(
				...
				LinearLayout.LayoutParams.MATCH_PARENT));
		if (TextUtils.isEmpty(calibrationPath)) {
			Toast.makeText(this, "null param,return!", Toast.LENGTH_LONG)
					.show();
			finish();
		} else {
			Toast.makeText(DSOForDataSetActivity.this, "init has been started!",
			...
			new Thread(new Runnable() {
				...
				public void run() {
					// TODO Auto-generated method stub
					DsoNdkInterface.initSystemWithParameters(calibrationPath);
							Log.e("information==========>",
	...
	Handler frameHandler = new Handler() {
        ...
        public void handleMessage(Message msg) {   
             switch (msg.what) {   
                  case UPDATE_MSG:   
                	  Toast.makeText(DSOForDataSetActivity.this,
								"init has been finished!",
								Toast.LENGTH_LONG).show();
          			new Thread(new Runnable() {
        					...
        					if (!TextUtils.isEmpty(ImgPath)) {
        						...
        						if (dir.isDirectory()) {
									for (int i=0; i< filenameL.length; i++)  {
										    ...
										    Bitmap currentL = BitmapFactory.decodeFile(filenameL[i].getAbsolutePath());
											Bitmap currentR = BitmapFactory.decodeFile(filenameR[i].getAbsolutePath());
											...
											int[] current_R = new int[w*h];

											currentL.getPixels(current_L, 0, w, 0, 0, w, h);
											currentR.getPixels(current_R, 0, w, 0, 0, w, h);
											DsoNdkInterface.insertFrame(timestamp,current_L,current_R,w,h);
									}
        						}
        					} else {
        						Toast.makeText(DSOForDataSetActivity.this, "empty images",
        								Toast.LENGTH_LONG).show();
        					}
        				}
        			}).start();
        				...
        				public void run() {
        					if (!TextUtils.isEmpty(ImgPath)) {
        						...
        						if (dir.isDirectory()) {
        							for (int i=0; i< filenameL.length; i++) {	
        								// TODO Auto-generated method stub
        								int[] resultInt = DsoNdkInterface.trackRealTime();
	...
	public void onSurfaceChanged(GL10 gl, int width, int height) {
		// TODO Auto-generated method stub
		DsoNdkInterface.glesResize(width,height);
	}

	@Override
	public void onDrawFrame(GL10 gl) {
		// TODO Auto-generated method stub
		DsoNdkInterface.glesRender();
	}
